...
# Substances --------------------------------------------------------------
                 tabPanel("Substances"),
                          ...
                          #     
                          #     # # testing
                          #     # selectInput("Substance",
                          #     #             label = "Choose one or more Substance(es) to display",
                          #     #             choices = Substancees,
                          #     #             selected = substancees[7]),
                          #     
                          #     checkboxGroupInput("Substance",
                          #                        # h3("Substance"),
                          #                        label = "Choose one or more substance(es) to display",
                          #                        choices = list("Deliriant",
                          #                                       "Dissociative",
                          ...
                          #   
                          #   mainPanel(
                          #     verbatimTextOutput("Substance_selected"),
                          #     plotOutput("studies_over_year_plot_Substance"),
                          #     # plotOutput("test_plot"),
                          #     dataTableOutput("table_print_Substance")
                          #   )
                          # )),
                 tabPanel("Map")
)
  ...
  output$studies_over_year_plot_Class <- renderPlot({
    ...
    p <- ggplot(data = PS.data.plot.Class,
                aes(x=Date, y=cumul_year_class,
                    # group = Class,
                    col = Class)) +
      # geom_bar(data = PS.data.plot.Class %>%
      #            filter(Class != "all"),
      #          aes(x=Date, y=one,
      #              group = Class),
      ...
      # scale_color_manual(values = c("Can. rec. ant."=unique(PS.data.plot.Class$col)[1],
      #                               "Deliriant"=unique(PS.data.plot.Class$col)[2],
      #                               "Dissociative"=unique(PS.data.plot.Class$col)[3],
      #                               "Endocannabinoids"=unique(PS.data.plot.Class$col)[4],
      #                               "Entactogen"=unique(PS.data.plot.Class$col)[5],
      #                               "Harmala alkaloids"=unique(PS.data.plot.Class$col)[6],
      #                               "Phytocannbinoids"=unique(PS.data.plot.Class$col)[7],
      #                               "Psychedelic"=unique(PS.data.plot.Class$col)[8],
      #                               "Synthetic cannabinoids"=unique(PS.data.plot.Class$col)[9])) +
      scale_color_manual(values = c(unique(PS.data.plot.Class$col))) +
      # scale_colour_viridis_d() +
      theme_bw()
    # geom_text(data = PS.data.plot.Class %>% 
    ...
    # grid.draw(gt)
    # plot(1:2, col=unique(PS.data.plot.Class$col),pch=16,cex=5)
    p
  })
